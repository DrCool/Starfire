You will be given a JSON object with two keys: `rooms` and `edges`.
- `rooms` contains `{ "id": "R001" }` objects (and maybe placeholder names).
- `edges` contains undirected edges `{ "a": "R001", "b": "R002" }` and sometimes `{ "a": "...", "b": "...", "kind":"vertical" }`.
  You MUST NOT change the graph: do not modify `edges` and do not add/remove rooms.

OUTPUT RULES
- Output JSON only. No markdown. No commentary.
- The output JSON must have exactly two top-level keys: `rooms` and `edges`.
- `edges` must be identical (same array items; same values) to the input `edges`.
- Each room object must contain exactly these fields (no extras):
    - `id` (string, unchanged)
    - `name` (string, 2–6 words)
    - `description` (string, <= 255 chars)
    - `verbose_description` (string, can be longer; use \n for paragraphs)
    - `inside` (0 or 1)
    - `outside` (0 or 1)
    - `terrain_type_id` (integer or null)
...
1) Ship Landing / Arrival Terminal (players enter the zone here)
2) Shop / Vendor
3) Bank / Storage
4) Medbay / Respawn
5) Training Hall
6) Bar / Cantina
7) Mission / Quest Desk
...
- Put Bank & Shop on or near the “main flow” (higher-traffic junctions).
...
- Use consistent sensory motifs (neon haze, humming conduits, ozone, holo-ads, service drones, old bulkheads, etc.).
...
Brightash is laid out around a central crossway where a north–south avenue intersects a major east–west boulevard. From this hub:
  • North leads toward cleaner, more industrial corridors and eventually the filtration systems of Blackflow (a zone to the north)
  • South transitions into crowded worker housing in Cinderblocks (zone to the south)
  • East runs through commerce and tower fronts toward a discreet landing terminal and the logistics routes of The Lower Yards (zone to the east)
  • West narrows into seedier alleys, banks, bars, and hidden maintenance paths leading toward The Ash Pits (zone to the west)
...
Important room types commonly found in Brightash include:
  ...
  • A training lofts above street level
...
{
  "rooms":[
    {"id":"R001","name":"...","description":"...","verbose_description":"...","inside":1,"outside":0,"terrain_type_id":null,"room_type_id":6},
    ...
  ],
  "edges":[ ...exactly the same edges... ]
}
```
